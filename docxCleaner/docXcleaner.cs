...
namespace docxCleaner
{
	internal class docXcleaner
	{
		...
		private const string KeyPrivacy = "-custom";
		...
		static void Main(string[] args)
		{
			...
			try
			{
				...
				using (ZipArchive archive = ZipFile.Open(backupFileName, ZipArchiveMode.Update))
				{
					...
					ProcessApp(archive);
					ProcessCore(archive, backupFileName);
					...
					if(commandlineArgs.ContainsKey(KeyCompress))
					{
						...
						{
							Console.WriteLine("Invalid quality value. Using default value of 75.");
		...
		private static void PrepareArguments(string[] args)
		{
			...
			commandlineArgs.TryAdd(KeyLanguage, "en-GB");
			commandlineArgs.TryAdd(KeyStyles, "");
			commandlineArgs.TryAdd(KeyPrivacy, "");
			commandlineArgs.TryAdd(KeyCompress, "75");
		...
		/// <summary>
		/// Processes the app.xml file in the given Zip archive.
		/// </summary>
		/// <param name="archive">The Zip archive.</param>
		private static void ProcessApp(ZipArchive archive)
		{
			...
			if (wordDocumentEntry != null)
			{
				...
				using (Stream stream = wordDocumentEntry.Open())
				{
					...
					if (node != null)
					{
						node.InnerText = "Vestas Wind Systems A/S";

						changed = true;
					}

					node = doc.SelectSingleNode("//ep:TitlesOfParts/vt:vector/vt:lpstr", nsmgr);
					if (node != null)
					{
						node.InnerText = "Document title";
		...
		/// <summary>
		/// Processes the core.xml file in the given Zip archive.
		/// </summary>
		/// <param name="archive">The Zip archive.</param>
		/// <param name="title">The new title for the document.</param>
		private static void ProcessCore(ZipArchive archive, string title)
		{
			...
			if (wordDocumentEntry != null)
			{
				...
				using (Stream stream = wordDocumentEntry.Open())
				{
					...
					XmlNode? node = doc.SelectSingleNode("//dc:title", nsmgr);
					if (node != null)
						node.InnerText = title;

					node = doc.SelectSingleNode("//dc:creator", nsmgr);
					if (node != null)
						node.InnerText = "Vestas CIR";
		...
		private static void ProcessContentTypes(ZipArchive archive)
		{
			...
			{
				Console.WriteLine("Found '[Content_Types].xml' file.");
				...
				using (Stream stream = contentTypes.Open())
				{
					...
					// find a Default node for JPG extension
